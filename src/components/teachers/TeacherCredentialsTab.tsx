...
interface TeacherCredentialsTabProps {
  teacherId: string;
}

export const TeacherCredentialsTab = ({ teacherId }: TeacherCredentialsTabProps) => {
  const [showPassword, setShowPassword] = useState(false);
  const [isResetting, setIsResetting] = useState(false);
  const queryClient = useQueryClient();

  const { data: credentials, isLoading } = useQuery({
    queryKey: ['teacher-credentials', teacherId],
    queryFn: async () => {
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['teacher-credentials', teacherId] });
    } catch (err: any) {
      toast({
        title: 'Reset Failed',
  ...
  if (!credentials) {
    return (
      <div className="flex flex-col items-center justify-center h-48 text-muted-foreground">
        ...
        <p className="text-xs mt-1">This teacher does not have login credentials yet.</p>
      </div>
    );
  }
    ...
    <Card className="border shadow-sm">
      <CardContent className="p-5 space-y-5">
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    This will generate a new temporary password for the teacher. The old password will no longer work. Are you sure?
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
        ...
        <p className="text-[11px] text-muted-foreground/60 italic">
          This information is sensitive. Only admins can view this.
        </p>
      </CardContent>
    </Card>
  );
