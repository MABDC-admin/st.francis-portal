...
export function useMessaging() {
  ...
  // Fetch conversations — batch queries to avoid N+1
  const fetchConversations = useCallback(async () => {
    ...
    try {
      ...
      // --- FIX: Batch fetch latest messages for ALL conversations (avoids N+1) ---
      ...
      // --- FIX: Batch calculate unread counts from fetched data ---
      const enrichedConvs: Conversation[] = convData.map((conv) => {
        const lastMsg = latestMessageMap[conv.id] || null;
        const convParticipants = (allParticipants || [])
          .filter(p => p.conversation_id === conv.id)
          .map(p => ({ ...p, profile: profileMap[p.user_id] }));

        // Calculate unread from the batch data
  ...
  useEffect(() => {
    ...
    // --- FIX: Typing presence uses a single channel stored in ref ---
  ...
  // --- FIX: Broadcast typing reuses the existing channel ref (no leak) ---
